...
Requires [Docker Desktop](https://www.docker.com/products/docker-desktop/) with virtualization enabled (WSL2 on Windows).
...
**First-time setup** — the database starts empty; roles must be seeded once:
...
Connect to the containerized MySQL at `127.0.0.1:3307` (not the default 3306, to avoid conflicting with any local MySQL installation) to run this.

Then register a user via `POST /api/v1/users`, and promote them to ADMIN via direct SQL (see `docs/bootstrap.sql` or the snippet below) — there's no UI for this first-admin bootstrap step by design, matching how most real systems require an initial manual admin seed.
...
- No automated role-seeding on first startup — currently a manual one-time SQL step (see Setup above). A `CommandLineRunner`-based seed would remove this.
